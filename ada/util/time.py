...
def timestamp(offset = None):
	...
	if offset:
		t += offset
	return t.strftime(config.util.time.format)

def is_expired(timestamp):
	t = datetime.strptime(timestamp, config.util.time.format)
	return t <= datetime.utcnow()

def from_timestamp(timestamp):
	return datetime.strptime(timestamp, config.util.time.format)
